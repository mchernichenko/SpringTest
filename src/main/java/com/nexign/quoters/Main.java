...
public class Main {
    public static void main(String[] args) throws InterruptedException {
        ...
        /* если провести параллель с corona, то там мы в раннер передавали имя пакета и мапу, которая затем передавалась в конфиг.
           ...
           В спринге используется конфиг ввиде xml, по которому строится схожая мапа BeanDefinitions, куда складируются все биклараци бинов
        ...
        // прогоняются через BeanPostProcessor и только после попадают в контейнет и отдаются контекстом. Задействован паттерн Chain of Responsibility
        while (true) {
            Thread.sleep(1000);
