...
public class Main
{
    public static void main( String[] args )
    {
         ...
         */
        // контекст создаётся через раннер - вспомогательный статический класс, а не напрямую, т.к. нужно создать не только создать контекст, но и фабрику и
        // еще связать фабрику и контекст между собой. Вот это небольшую логику запихали в раннер.
        ...
        // запрашиваем основной объект у контекста, чтобы запустить основную логику программы.
        // Все остальные объекты, от которых зависит CoronaDesinfector уже создаются фабрикой с помощью конфигураторов, через которые прогоняется созданный объект
        // конфигурирование производится с помощью аннотаций (инъекций), который обрабатывают конфигураторы.
