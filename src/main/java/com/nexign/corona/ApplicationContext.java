...
public class ApplicationContext {
    ...
    public <T> T getObject(Class<T> type) {
        ...
        // достаём имплементацию по переданому интерфейсу
        if (implClass.isInterface()) { // т.к. нам нужна конкретрая реализация, то проверяем, что если переданнный тип является интерфейсом, то
