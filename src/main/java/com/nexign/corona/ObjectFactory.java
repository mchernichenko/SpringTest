...
public class ObjectFactory {
     ...
     * @param <T> тип позвращаемого объекта
     * @return возващает экземляр объекта типа <T>
     */
    @SneakyThrows
    public <T> T createObject(Class<T> implClass) {
        ...
        // todo Вся магия по настройке объекта тут. Т.е. перед тем как отдать объект, его можно настроить согласно нашим конвенциям, которые мы придумаем.

        /* ничего страшного в том, что некоторые конфигураторы отработают вхолостую, т.е. ничего по факту не настроят,
           например, не укаждого объекта филды инициализируются с помощью аннотации InjectProperty. Это разовая, зато универсальная настройка.
         */
        ...
        // если нужно, то проксируем объект
